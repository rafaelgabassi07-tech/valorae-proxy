...
export default async function handler(req, res) {
  const route = beginRoute(req, res, { version: ValoraeEngine.version, methods: ['GET', 'POST'], route: 'portfolio-risk', rateMax: Number(process.env.VALORAE_RATE_LIMIT_PORTFOLIO_MAX || 60), profile: 'portfolio' });
  ...
  try {
    ...
    return sendJson(req, res, { requestId: route.requestId, ...{version:data.version,engineVersion:data.engineVersion,status:data.status,generatedAt:data.generatedAt,summary:data.summary,risk:data.risk,allocation:data.allocation,technologyReadiness:data.intelligence?.technologyReadiness,dataCompleteness:data.intelligence?.dataCompleteness,insights:data.insights,diagnostics:data.diagnostics} }, { status: data.status === 'EMPTY' ? 400 : 200, engineVersion: ValoraeEngine.version, profile: 'portfolio', cacheControl: 'private, max-age=10, stale-while-revalidate=60' });
  } catch (err) {
