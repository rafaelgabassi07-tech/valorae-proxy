...
function sendMethodNotAllowed(res, allow = 'GET, POST, OPTIONS') {
  res.setHeader('Allow', allow);
  return res.status(405).json({ error: `Método não permitido. Use ${allow.replace('OPTIONS', '').replace(/,\s*$/, '').trim()}.` });
}
